...
<script lang="ts">
  ...
  // Form data
  let content: string = '';
  let scheduledTime: Date = new Date(initialDate);
  let selectedPlatforms: PlatformType[] = ['twitter'];
  let hashtags: string[] = [];
  let formattedDate: string = '';
  let formattedTime: string = '';
  let isSubmitting: boolean = false;
  let aiGenerating: boolean = false;
  ...
  // AI suggestions - these would come from your AI service in a real app
  const aiContentSuggestions = [
    "Excited to announce our latest feature release! Check out how we've improved our user experience. #ProductUpdate #Innovation",
    "Looking for tips to boost your productivity? Here are 3 simple techniques that helped our team increase efficiency by 40% this month.",
    "Happy Monday! Start your week with inspiration: 'The only way to do great work is to love what you do.' - Steve Jobs #MondayMotivation"
  ];
  ...
  // Format date and time on mount and when initialDate changes
  $: if (initialDate) {
    scheduledTime = new Date(initialDate);
    formatDateTime();
  }
  ...
  // Generate AI content (simulated here)
  function generateAiContent() {
    aiGenerating = true;
    
    // Simulate AI generation delay
    setTimeout(() => {
      const randomIndex = Math.floor(Math.random() * aiContentSuggestions.length);
      content = aiContentSuggestions[randomIndex];
      aiGenerating = false;
    }, 1500);
  }
  ...
  async function submitForm() {
    ...
    if (remainingChars < 0) {
      errorMessage = 'Content exceeds character limit';
      return;
    }
  ...
  >
    ...
    >
      ...
      <div class="p-4 overflow-y-auto flex-1">
        ...
        <div class="mb-4">
          <button 
            class="w-full flex items-center justify-center px-4 py-2 bg-purple-100 text-purple-800 rounded-md hover:bg-purple-200 disabled:opacity-50 disabled:cursor-not-allowed"
            on:click={generateAiContent}
            disabled={aiGenerating}
          >
            {#if aiGenerating}
              <span class="inline-block animate-spin h-4 w-4 border-2 border-purple-600 rounded-full border-t-transparent mr-2"></span>
              Generating...
            {:else}
        ...
        <div class="mb-4">
          ...
          <div class="space-y-2">
            {#each aiContentSuggestions as suggestion, i}
              <div 
                class="p-3 border border-gray-200 rounded-md hover:bg-gray-50 cursor-pointer"
                on:click={() => useAiSuggestion(suggestion)}
              >
                <div class="text-sm truncate">{suggestion}</div>
              </div>
            {/each}
          </div>
        </div>
      ...
      <div class="p-4 border-t flex justify-end space-x-2">
        ...
        <button 
          class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-blue-300 disabled:cursor-not-allowed flex items-center"
          on:click={submitForm}
          disabled={content === '' || selectedPlatforms.length === 0 || remainingChars < 0 || isSubmitting}
        >
          {#if isSubmitting}
